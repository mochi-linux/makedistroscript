#!/usr/bin/env bash
# =============================================================================
#  MochiLinux Cross-Compilation Toolchain Builder
#  Builds: Linux 7.1.3, glibc 2.43, GCC 16.1.0, Binutils 2.46.1, Bash 5.3,
#          Coreutils 9.11, Readline 8.3, Ncurses 6.6, Inetutils 2.7,
#          sed 4.9, gawk 5.4.0, m4 1.4.21, diffutils 3.12, findutils 4.10.0, grep 3.12
# =============================================================================
set -euo pipefail
trap 'echo -e "\n${RED:-}[✗]${RESET:-} Build failed at line ${LINENO}. Check the latest stage log under ${BUILD_DIR:-<build-dir-not-set>}." >&2' ERR
...
export TARGET="${TARGET:-$(uname -m)-mochilinux-linux-gnu}"

setup_dirs() {
  DOWNLOAD_DIR="${PROJECT_DIR}/cache/distfiles"
  SOURCES_DIR="${PROJECT_DIR}/sources"
  BUILD_DIR="${PROJECT_DIR}/build/${TARGET}"
  ARTIFACTS_DIR="${PROJECT_DIR}/artifacts"
  export SYSROOT="${ARTIFACTS_DIR}/rootfs"
  TOOLCHAIN_DIR="${ARTIFACTS_DIR}/toolchain"   # cross tools live here (not installed to host or rootfs)
  STAMPS_DIR="${PROJECT_DIR}/.stamps/${TARGET}"
}
...
prompt_project_dir() {
  ...
  echo -e "${BOLD}${CYAN}══════════════════════════════════════════${RESET}"
  echo -e "  ${BOLD}1)${RESET} ${HOME}/MochiLinux  ${CYAN}(default)${RESET}"
  ...
  case "${_choice}" in
    ...
    3)
      ...
      export PROJECT_DIR="${_custom:-${HOME}/MochiLinux}"
      ;;
    *)
      export PROJECT_DIR="${HOME}/MochiLinux"
      ;;
  esac
...
# ─── Source Mirrors / URLs ────────────────────────────────────────────────────
# Cyberbits is the default GNU mirror because it is usually faster from Thailand.
# Override at runtime if needed:
#   GNU_MIRROR=https://ftpmirror.gnu.org ./buildrootfs_mochilinux.sh
GNU_MIRROR="${GNU_MIRROR:-https://mirror.cyberbits.asia/gnu}"
KERNEL_MIRROR="${KERNEL_MIRROR:-https://cdn.kernel.org/pub/linux/kernel}"

GNU_FALLBACK_MIRRORS=(
...
declare -A URLS=(
  ["linux-${VER_LINUX}.tar.xz"]="${KERNEL_MIRROR}/v7.x/linux-${VER_LINUX}.tar.xz"
  ["glibc-${VER_GLIBC}.tar.xz"]="${GNU_MIRROR}/glibc/glibc-${VER_GLIBC}.tar.xz"
  ["gcc-${VER_GCC}.tar.xz"]="${GNU_MIRROR}/gcc/gcc-${VER_GCC}/gcc-${VER_GCC}.tar.xz"
  ["binutils-${VER_BINUTILS}.tar.xz"]="${GNU_MIRROR}/binutils/binutils-${VER_BINUTILS}.tar.xz"
  ["bash-${VER_BASH}.tar.gz"]="${GNU_MIRROR}/bash/bash-${VER_BASH}.tar.gz"
  ["coreutils-${VER_COREUTILS}.tar.xz"]="${GNU_MIRROR}/coreutils/coreutils-${VER_COREUTILS}.tar.xz"
  ["readline-${VER_READLINE}.tar.gz"]="${GNU_MIRROR}/readline/readline-${VER_READLINE}.tar.gz"
  ["ncurses-${VER_NCURSES}.tar.gz"]="${GNU_MIRROR}/ncurses/ncurses-${VER_NCURSES}.tar.gz"
  ["inetutils-${VER_INETUTILS}.tar.gz"]="${GNU_MIRROR}/inetutils/inetutils-${VER_INETUTILS}.tar.gz"
  ["sed-${VER_SED}.tar.xz"]="${GNU_MIRROR}/sed/sed-${VER_SED}.tar.xz"
  ["gawk-${VER_GAWK}.tar.xz"]="${GNU_MIRROR}/gawk/gawk-${VER_GAWK}.tar.xz"
  ["m4-${VER_M4}.tar.xz"]="${GNU_MIRROR}/m4/m4-${VER_M4}.tar.xz"
  ["diffutils-${VER_DIFFUTILS}.tar.xz"]="${GNU_MIRROR}/diffutils/diffutils-${VER_DIFFUTILS}.tar.xz"
  ["findutils-${VER_FINDUTILS}.tar.xz"]="${GNU_MIRROR}/findutils/findutils-${VER_FINDUTILS}.tar.xz"
  ["grep-${VER_GREP}.tar.xz"]="${GNU_MIRROR}/grep/grep-${VER_GREP}.tar.xz"
)
...
step_install_deps() {
  ...
  if command -v pacman &>/dev/null; then
    local PKGS=(
      ...
      patch help2man autoconf automake libtool diffutils file
    )
...
step_download() {
  ...
  for TARBALL in "${!URLS[@]}"; do
    ...
    # Add GNU fallback mirrors for aria2c. aria2 will try these mirrors for the
    # same output file, which makes repeated curl/pipe rebuilds much less cursed.
    if [[ "${URL}" == "${GNU_MIRROR}/"* ]]; then
      local GNU_PATH="${URL#${GNU_MIRROR}/}"
      for MIRROR in "${GNU_FALLBACK_MIRRORS[@]}"; do
        echo "  ${MIRROR}/${GNU_PATH}" >> "${INPUT_FILE}"
      done
    fi
...
step_prepare_env() {
  ...
  mkdir -p "${BUILD_DIR}"/{binutils-stage1,gcc-stage1,glibc-headers,glibc,gcc-stage2,libgcc}

  # Create cross-toolchain directories (host environment)
  mkdir -p "${TOOLCHAIN_DIR}/bin" "${ARTIFACTS_DIR}" "${SYSROOT}" "${BUILD_DIR}"
...
step_gcc_stage1() {
  ...
  log "Building GCC Stage 1 compiler only (${JOBS} jobs)..."
  make -j"${JOBS}" all-gcc \
    2>&1 | tee "${BLD}/build.log"

  log "Installing GCC Stage 1 compiler only..."
  make install-gcc \
...
step_glibc_headers() {
  ...
  mkdir -p "${SYSROOT}/usr/include" "${SYSROOT}/usr/lib" "${SYSROOT}/lib" "${SYSROOT}/lib64"
...
# =============================================================================
#  STEP 9b — Build target libgcc after glibc headers/startfiles
# =============================================================================
step_libgcc() {
  step "STEP 9b — Building target libgcc with glibc headers/startfiles"
  is_done "libgcc" && { info "Already done — skipping."; return 0; }

  local BLD="${BUILD_DIR}/gcc-stage1"
  [[ -d "${BLD}" ]] || die "GCC Stage 1 build directory not found: ${BLD}"

  cd "${BLD}"

  log "Building target libgcc (${JOBS} jobs)..."
  make -j"${JOBS}" all-target-libgcc \
    2>&1 | tee "${BUILD_DIR}/libgcc/build.log"

  log "Installing target libgcc..."
  make install-target-libgcc \
    2>&1 | tee "${BUILD_DIR}/libgcc/install.log"

  log "target libgcc complete ✓"
  mark_done "libgcc"
}
...
# =============================================================================
#  STEP 23 — Finalize sysroot layout
# =============================================================================
step_finalize_layout() {
  step "STEP 23 — Finalizing MochiLinux sysroot layout"
  is_done "finalize-layout" && { info "Already done — skipping."; return 0; }

  mkdir -p "${SYSROOT}/lib64" "${SYSROOT}/usr/bin" "${SYSROOT}/usr/lib"

  # x86_64 dynamic loader compatibility. Some glibc configurations install
  # ld-linux under /lib; keep /lib64 available without hard failing.
  if [[ -f "${SYSROOT}/lib/ld-linux-x86-64.so.2" && ! -e "${SYSROOT}/lib64/ld-linux-x86-64.so.2" ]]; then
    mv -v "${SYSROOT}/lib/ld-linux-x86-64.so.2" "${SYSROOT}/lib64/"
  fi

  if [[ -f "${SYSROOT}/usr/bin/bash" && ! -e "${SYSROOT}/bin" ]]; then
    ln -sfv usr/bin "${SYSROOT}/bin" 2>/dev/null || true
  fi

  log "Sysroot layout finalized ✓"
  mark_done "finalize-layout"
}

# =============================================================================
#  STEP 24 — Create artifacts
# =============================================================================
step_create_artifacts() {
  step "STEP 24 — Creating MochiLinux artifacts"
  is_done "artifacts" && { info "Already done — skipping."; return 0; }

  mkdir -p "${ARTIFACTS_DIR}/release"
  local DATE_TAG
  DATE_TAG="$(date +%Y%m%d)"
  local ROOTFS_ARTIFACT="${ARTIFACTS_DIR}/release/mochilinux-rootfs-${TARGET}-${DATE_TAG}.tar.xz"
  local TOOLCHAIN_ARTIFACT="${ARTIFACTS_DIR}/release/mochilinux-toolchain-${TARGET}-${DATE_TAG}.tar.xz"

  log "Packing rootfs: ${ROOTFS_ARTIFACT}"
  tar -C "${SYSROOT}" --numeric-owner -cJf "${ROOTFS_ARTIFACT}" .

  log "Packing toolchain: ${TOOLCHAIN_ARTIFACT}"
  tar -C "${TOOLCHAIN_DIR}" --numeric-owner -cJf "${TOOLCHAIN_ARTIFACT}" .

  log "Artifacts complete ✓"
  log "Rootfs artifact    : ${ROOTFS_ARTIFACT}"
  log "Toolchain artifact : ${TOOLCHAIN_ARTIFACT}"
  mark_done "artifacts"
}
...
main() {
  ...
  # Step 9 & 10: C Library bootstrap + full glibc
  step_glibc_headers
  step_libgcc
  step_glibc_full
  ...
  step "🎉 MochiLinux Toolchain and Base System Build Complete! 🎉"
  log "Project Directory: ${PROJECT_DIR}"
  log "Sysroot is available at: ${SYSROOT}"
  log "Cross-tools are at: ${TOOLCHAIN_DIR}"
  log "Artifacts are at: ${ARTIFACTS_DIR}/release"
}
